...
class BlockChain:
    ...
    def start(self):
        ...
        while(True):
            ...
            #    cur = con.cursor()
            #    for row in rows:
            #        cur.execute("update usertransaction SET count=? where sender=?",[int(row[4])+1,row[0]])
            
            #con.commit()
            #tr=Transaction("none","none",0,0,"none")
            #for row in rows:
            #    tr.sender=row[0]
            #    tr.receiver=row[1]
            #    tr.amounts=row[2]
